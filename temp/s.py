...
def analyze_hc_G_scaling():
    """
    Analyze how hc and G relate when we scale kg to make their ratio 1 using Decimal for precision.
    """
    # Original constants
    ...
    print()
    #alpha = hc_new**(1/3)
    alpha = hc_new ** (Decimal(1) / Decimal(3))
    print(f"Meter scaling factor (alpha) = {alpha:.50e}")
    print(f"1/ Meter scaling factor (alpha) = {Decimal(1) / alpha:.20e}")
    print(f"Meter/c scaling factor (alpha) = {alpha / c:.20e}")
    print(f"c/Meter scaling factor (alpha) = {c / alpha:.20e}")

    beta = kg_scale
    print(f"\nkg scaling factor (beta)     = {beta:.50e}")
    print(f"1/kg scaling factor (beta)     = {Decimal(1) / beta:.20e}")
    ...
    print(f"1 kg in new units = {Decimal(1) / kg_scale:.8e} kg in original units")
